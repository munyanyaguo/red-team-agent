...
def auth_required(roles=None):
    ...
    Args:
        roles (list or callable): List of roles allowed to access the endpoint (e.g., ['admin', 'analyst'])
                     If None, any authenticated user can access

    Usage:
        @auth_required  # Any authenticated user (no parentheses)
    ...
    """
    # Support usage without parentheses: @auth_required
    if callable(roles):
        fn = roles
        return _auth_decorator(fn, None)

    def decorator(fn):
        return _auth_decorator(fn, roles)
    return decorator


def _auth_decorator(fn, roles):
    """Internal decorator implementation for auth_required."""
    @wraps(fn)
